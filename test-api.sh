...
RESPONSE=$(curl -s -w "%{http_code}" -X POST -H "Content-Type: application/json" -d "$NEW_CAR" $BASE_URL)
HTTP_CODE=${RESPONSE: -3}
BODY=${RESPONSE:0:${#RESPONSE}-3}
...
if [ "$HTTP_CODE" -eq 200 ] || [ "$HTTP_CODE" -eq 201 ]; then
    ...
    if [[ $BODY =~ \"carId\":([0-9]+) ]]; then
        ...
        FIRST_CAR=$(curl -s $BASE_URL | grep -o '"carId":[0-9]*' | head -1 | cut -d':' -f2)
        if [ -n "$FIRST_CAR" ]; then
            CAR_ID=$FIRST_CAR
...
RESPONSE=$(curl -s -w "%{http_code}" $BASE_URL/$CAR_ID)
HTTP_CODE=${RESPONSE: -3}
BODY=${RESPONSE:0:${#RESPONSE}-3}
...
RESPONSE=$(curl -s -w "%{http_code}" -X PUT -H "Content-Type: application/json" -d "$UPDATE_CAR" $BASE_URL/$CAR_ID)
HTTP_CODE=${RESPONSE: -3}
BODY=${RESPONSE:0:${#RESPONSE}-3}
...
RESPONSE=$(curl -s -w "%{http_code}" -X DELETE $BASE_URL/$CAR_ID)
HTTP_CODE=${RESPONSE: -3}
BODY=${RESPONSE:0:${#RESPONSE}-3}
